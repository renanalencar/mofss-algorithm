...
public class SolutionSet implements Serializable {
  ...
  */
  ...
  /* Acrescentada esta função para poder escrever em arquivo as métricas 
  ...
  public void printQualityIndicatorToFile(String path, String[] name, double[] value){
    try {
      ...
      for (int i=0; i<name.length; i++){
    	  bw.write(name[i] + "\t" + deci.format(value[i]));
    	  bw.newLine();
      }
  ...
  /* Acrescentada esta função para poder escrever em arquivo as quantidade de partículas
   * dominadas, não-dominadas e indiferentes relativas ao movimento individual 
   * Augusto Guimaraes 28.01.2013 */
  ...
  public void printFitnessToFile(String path){
    try {
      ...
      for (int i = 0; i < solutionsList_.size(); i++) {
          //if (this.vector[i].getFitness()<1.0) {
          bw.write("" + solutionsList_.get(i).getFitness());
          bw.newLine();
          //}
      }
