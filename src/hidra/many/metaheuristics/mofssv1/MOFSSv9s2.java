 ...
 * 
 * 
 *     VERS�O sem DeltaF (substituido por DeltaW)
 *     
 *     
 * 
 */
public class MOFSSv9s2 extends Algorithm {
  ...
  public MOFSSv9s2(Problem problem) {
    super (problem) ;
  ...
  private void calculateIndivOper() throws JMException, ClassNotFoundException{
	  ...
	  for (int i=0; i<swarmSize_; i++){ //para cada peixe do cardume
		  ...
		  //double euclideanDistance = euclideanDistance(olderParticle, front.get(leadersRandom));
		  ...
		  if (euclideanDistance == 0){
			  ...
			  for (int j=0; j<problem_.getNumberOfVariables(); j++){ //para cada dimens�o de cada peixe
				  ...
				  //System.out.println("Delta ["+i+"]["+j+"] = " + delta_x_[i][j] + " -> "+movement +"*"+step_+"*"+rand);
			  ...
			  if (flag != 0){
				  ...
				  double euclideanDistanceNewPosition = euclideanDistance(particleNew, front.get(leadersRoletteWheel));
				  ...
				  System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
				  System.out.println("CD old: " + crowdingDistanceOldPosition + " | CD new: " + crowdingDistanceNewPosition);
				  System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
				  
				  if (euclideanDistanceNewPosition < euclideanDistance){
					  for (int j=0; j<problem_.getNumberOfVariables(); j++){
						  particle.setValue(j, particleNew.getDecisionVariables()[j].getValue()) ;
					  }
				  }else{
  ...
  private void calculateFeedOper() {
	  ...
	  for (int i=0; i<swarmSize_; i++){
		  ...
		  if (particles_.get(i).getRank() != 0){
			  ...
			  //particles_.get(i).setFitness(particles_.get(i).getFitness() - 2);
		  ...
		  delta_w_ += particles_.get(i).getRank(); 
  ...
  private void calculateColInstOper() throws JMException{
	  ...
	  /*for (int i = 0; i < problem_.getNumberOfVariables(); i++) {
		  System.out.println("I["+i+"] = " + imDirection[i]);
	  }*/
	  ...
	  for (int i=0; i<swarmSize_; i++){
		  ...
		  for (int j=0; j<problem_.getNumberOfVariables(); j++){
			  //if (countD_ >= countND_){
			  	  ...
			  	  //System.out.println("p["+i+"]["+j+"] "+movement + " = " + stepVol + " * " + ((countD_-countND_)/swarmSize_) + " * " + rand + " * " + ((particle.getValue(j) - barycenter_.getDecisionVariables()[j].getValue())/euclideanDistance));
				  
			  	  particle.setValue(j,particle.getValue(j) - movement);
				  
				  /*if ((countD_ >= countND_) && (countIND_ < swarmSize_*0.75)){
					  particle.setValue(j,particle.getValue(j) - movement);
				  }else{
					  particle.setValue(j,particle.getValue(j) + movement);
				  }*/
  ...
  private void pertubationLeaders() throws ClassNotFoundException, JMException{
	  int leadersRandom = (int) (Math.random()*leaders_.size());
	  
	  Solution particleNew = new Solution (problem_);
	  Solution olderParticle = new Solution (leaders_.get(leadersRandom));
	  
	  double rand = PseudoRandom.randDouble(0,1);
	  double movement = rand * PseudoRandom.randDouble(problem_.getLowerLimit(0), problem_.getUpperLimit(0));
	  
	  for (int j=0; j<problem_.getNumberOfVariables(); j++){
		  
		  particleNew.getDecisionVariables()[j].setValue(movement + olderParticle.getDecisionVariables()[j].getValue()); 
	  ...
	  problem_.evaluate(particleNew);
	  int flag = dominance_.compare(particleNew, olderParticle);
	  if (flag == -1) {    
		  System.out.println("@@@@@@@@@@@ Pertubation Leaders @@@@@@@@@@@");
		  
		  XReal particle = new XReal (leaders_.get(leadersRandom));
  ...
  public SolutionSet execute() throws JMException, ClassNotFoundException {
	  ...
	  while(iterations_ <= maxIterations_){
		  ...
		  // Cria uma perturba��o (movimento aleat�rio) no arquivo externo
		  //pertubationLeaders();
		  ...
		  totalteste = (int) (totalteste + countIND_);
		  System.out.println("Indiferentes movidos: " + teste + "/" + totalteste);
