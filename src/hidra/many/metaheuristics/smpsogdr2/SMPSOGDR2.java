...
package hidra.many.metaheuristics.smpsogdr2;
...
import hidra.many.metaheuristics.fitness.PropostFitness;
import hidra.many.metaheuristics.fitness.SumWeightedFitness;
 ...
 */
public class SMPSOGDR2 extends HIDRAPopulationAlgorithm {
 ...
 // private int maxIterations_;
   ...
   */
  private Comparator comparatorFitness_;
   ...
   */
  public SMPSOGDR2(Problem problem) {
    super(problem) ;
  ...
  public void initParams() {
    ...
    leaders_ = new BoundArchive(archiveSize_, problem_.getNumberOfObjectives(),new FitnessComparator(),new PropostFitness());

    // Create comparators for dominance and crowding distance
    dominance_ = new DominanceComparator();
    comparatorFitness_ = new FitnessComparator();
  ...
  private void computeSpeed(int iter, int miter) throws JMException {
    ...
    for (int i = 0; i < swarmSize_; i++) {
      ...
      if (comparatorFitness_.compare(one, two) > 0 ) {
...
*/  
  ...
  protected SolutionSet initializationAlgorithm() throws ClassNotFoundException,
	  ...
	  initParams();
	    ...
	    //Assign crowding distance to the leaders_
		  //Crowding the leaders_
		Fitness fitness = new PropostFitness();
  ...
  protected SolutionSet runIteration() throws JMException {
	      ...
	      for (int i = 0; i < particles_.size(); i++) {
	    	 ...
	    	 int flag = comparatorFitness_.compare(particles_.get(i), best_[i]);
		     if (flag == 1) { // the new particle is best_ than the older remeber        
		          Solution particle = new Solution(particles_.get(i));
		          best_[i] = particle;
		      }
