...
public class SpeaGDFitness {
   ...
   */
  public SpeaGDFitness(SolutionSet solutionSet) {
  ...
  /*  //Calculate the strength value
    ...
    SolutionSet copy1 = Util.copySolutionSet(solutionSet_);
    copy1.sort(new FitnessComparator());
    double minFitness = copy1.get(0).getFitness();
    
    for (int i = 0;i < solutionSet_.size(); i++) {
    	
    	double fitness = solutionSet_.get(i).getFitness();
    	solutionSet_.get(i).setFitness(fitness/minFitness);
    	
    }
    
    SolutionSet solutionSetDensity = Util.copySolutionSet(solutionSet_);
    
    GDFitness gdFitness = new GDFitness();
    gdFitness.fitnessAssign(solutionSet_, solutionSet_.get(0).numberOfObjectives()  );
    
    
    SolutionSet copy2 = Util.copySolutionSet(solutionSet_);
    copy2.sort(new FitnessComparator());
    minFitness = copy2.get(0).getFitness();
    ...
    for (int i = 0;i < solutionSet_.size(); i++) {
    	...
    	double fitness = solutionSet_.get(i).getFitness();
    	fitness = fitness + solutionSetDensity.get(i).getFitness();
    	solutionSet_.get(i).setFitness(fitness);
