 ...
 */
public class SMPSOMS extends HIDRAPopulationAlgorithm {
 ...
 // private int maxIterations_;
   ...
   */
  private Comparator comparatorFitness;
   ...
   */
  public SMPSOMS(Problem problem) {
    super(problem) ;
  ...
  public void initParams() {
    ...
    leaders_ = 
    		new BoundArchive(archiveSize_, 
    				problem_.getNumberOfObjectives(), 
    					new MSOPSComparator(dual,numTargets), 
    						new MSOPSFitness(this.weights,numTargets,dual));

    
    
    // Create comparators for dominance and crowding distance
    dominance_ = new DominanceComparator2();
    comparatorFitness = new MSOPSComparator(dual,numTargets);
	      ...
	      for (int i = 0; i < particles_.size(); i++) {
	        ...
	        if (flag == -1) { // the new particle is best_ than the older remeber        
	          ...
	          best_[i] = particle;
	        	...
	        	/*Solution particle = new Solution(particles_.get(i));
	        	fitness = new MSOPSFitness(this.weights,numTargets,dual);
	        	double fitness1 = fitness.globalDetritment(particle,  best_[i], problem_.getNumberOfObjectives());
	        	//particle.setFitness(fitness1);
	        	double fitness2 = fitness.globalDetritment(best_[i],particle,problem_.getNumberOfObjectives());
