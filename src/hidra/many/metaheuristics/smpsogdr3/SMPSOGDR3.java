...
package hidra.many.metaheuristics.smpsogdr3;
...
import hidra.many.metaheuristics.fitness.CrowdingDistanceFitness;
import hidra.many.metaheuristics.fitness.GDCDRFitness;
 ...
 */
public class SMPSOGDR3 extends HIDRAPopulationAlgorithm {
 ...
 // private int maxIterations_;
   ...
   */
  private Comparator<Solution> comparatorFitness;
   ...
   */
  public SMPSOGDR3(Problem problem) {
    super(problem) ;
  ...
  public void initParams() {
    ...
    leaders_ = new BoundArchive(archiveSize_, problem_.getNumberOfObjectives(),new FitnessComparator(),new GDCDRFitness());

    // Create comparators for dominance and crowding distance
    dominance_ = new DominanceComparator2();
    comparatorFitness = new FitnessComparator();
  ...
  private void computeSpeed(int iter, int miter) throws JMException {
    ...
    for (int i = 0; i < swarmSize_; i++) {
      ...
      if (comparatorFitness.compare(one, two) > 0 ) {
      ...
      for (int var = 0; var < particle.getNumberOfDecisionVariables(); var++) {
        ...
        speed_[i][var] = velocityConstriction(constrictionCoefficient(C1, C2) *
          (/*inertiaWeight(iter, miter, wmax, wmin)  */ W *
          speed_[i][var] +
...
*/  
  ...
  protected SolutionSet initializationAlgorithm() throws ClassNotFoundException,
	  ...
	  initParams();
	    ...
	    //assignFitness();
	    GDCDRFitness fitness = new GDCDRFitness();  
	    fitness.fitnessAssign(this.leaders_, problem_.getNumberOfObjectives());
...
/*private void assignFitness() {
	//Crowding the leaders_
	//SolutionSet copy1 = Util.copySolutionSet(this.leaders_);
	GDFitness fitness = new GDFitness();
	fitness.fitnessAssign(this.leaders_, problem_.getNumberOfObjectives());

	//SolutionSet copy2 = Util.copySolutionSet(this.leaders_);
	CrowdingDistanceFitness crowdingDistanceFitness  = new CrowdingDistanceFitness();
	crowdingDistanceFitness.fitnessAssign(this.leaders_, problem_.getNumberOfObjectives());
	
	if(this.leaders_.size()  == 1){
		this.leaders_.get(0).setFitness(0.0);
	}
	...
	double lowerCDFItness = copy2.get(0).getCrowdingDistance();
	
	for(int i=0; i < this.leaders_.size() ;i++){
		double fitness1 = this.leaders_.get(i).getFitness();
		double fitness2 = this.leaders_.get(i).getCrowdingDistance();
		
		
		fitness1 = fitness1 / lowerGDFitness;
		fitness2 = fitness2 / lowerCDFItness;
		
		this.leaders_.get(i).setFitness(fitness1 + fitness2);
	}
}*/
  ...
  /*private void assignFitness() {
		SolutionSet copy2 = Util.copySolutionSet(this.leaders_);
		CrowdingDistanceFitness crowdingDistanceFitness  = new CrowdingDistanceFitness();
		crowdingDistanceFitness.fitnessAssign(copy2, problem_.getNumberOfObjectives());
		...
		for(int i=0; i < this.leaders_.size() ;i++){
			...
			double fitness2 = copy2.get(i).getCrowdingDistance();			
			this.leaders_.get(i).setFitness(fitness2);
		}
	      ...
	      for (int i = 0; i < particles_.size(); i++) {
	        ...
	        if (flag == -1) { // the new particle is best_ than the older remeber        
	          ...
	          best_[i] = particle;
	        	...
	        	double fitness1 = gdFitness.globalDetritment(particle,  best_[i], problem_.getNumberOfObjectives());
	        	//particle.setFitness(fitness1);
	        	double fitness2 = gdFitness.globalDetritment(best_[i],particle,problem_.getNumberOfObjectives());
	        	//best_[i].setFitness(fitness2);	        		        	
	        	if(fitness1 < fitness2){// the new particle is best_ than the older remeber        	  	         
		          best_[i] = particle;	        		
	        	}*/
