 ...
 */
public class HIDRARunExperiment{

	public HIDRAExperiment experiment_ ;
	public int id_ ;
	...
	public HIDRARunExperiment(HIDRAExperiment experiment, 
		...
		for (int problemId = first_; problemId <= last_; problemId++) {
			...
			for (int runs = 0; runs < independentRuns_; runs++) {
				...
				for (int i = 0; i < numberOfAlgorithms; i++) {
					...
					*/
					...
					String pathDiretoryFinal = directory + "/final";
					File directoryFinal = new File(pathDiretoryFinal);
					
					if (!directoryFinal.exists()) {
						boolean result = new File(pathDiretoryFinal).mkdirs();
						System.out.println("Creating " + pathDiretoryFinal);
					}/*else{
						Util.removeDirectory(directoryFinal);
						boolean result = new File(pathDiretoryFinal).mkdirs();
					}*/
					
					
					String tempPath  =  directory + "/run"  + runs;
					File runDiretory =  new File(tempPath); 
					
					if (!runDiretory.exists()) {
						boolean result = new File(tempPath).mkdirs();
						System.out.println("Creating " + tempPath);						
					}
					
					
					/*
					String pathMetric  =  directory + "/metric";
					File fileMetric =  new File(pathMetric); 
					
					if (!fileMetric.exists()) {
						boolean result = new File(pathMetric).mkdirs();
						System.out.println("Creating " + tempPath);						
					}
					*/
					...
					try {
						try {
							...
							algorithm[i].setInputParameter("pathParetoFront", tempPath);
							algorithm[i].setInputParameter("pathDiretoryFinal", pathDiretoryFinal);
							//algorithm[i].setInputParameter("pathMetric", pathMetric);
							//algorithm[i].setInputParameter("run", runs);
							resultFront = algorithm[i].execute();
							
							/*if(GlobalSettings.CALCULATE_METRICS){
								
								Double hypervolume = (Double) algorithm[i].getOutputParameter("HV");
								Double convergenceMeasure = (Double) algorithm[i].getOutputParameter("CM");
								Double convergenceMeasure = (Double) algorithm[i].getOutputParameter("CM");
								
								
								Util.printData(hypervolume, pathDiretoryFinal + "/HV");
							
							}*/
							
							
							//deltaTime = System.currentTimeMillis() - time0;
						...
						Logger.getLogger(HIDRAExperiment.class.getName()).log(Level.SEVERE, null, ex);
					}

					// STEP 8: put the results in the output directory
					resultFront.printObjectivesToFile(pathDiretoryFinal + "/" + outputParetoFrontFile_ + "." + (runs+1));
					resultFront.printVariablesToFile(pathDiretoryFinal + "/" + outputParetoSetFile_ + "." + (runs+1));
			...
			/*		if (indicatorList_.length > 0) {
						...
						for (int j = 0; j < indicatorList_.length; j++) {
							...
							if (indicatorList_[j].equals("GEN_SPREAD")) {
								FileWriter os = null;
								try {
									double value = indicators.getGeneralizedSpread(resultFront);
									os = new FileWriter(experimentDirectory + "/GEN_SPREAD", true);
							...
							if (indicatorList_[j].equals("TM")) {
								FileWriter os = null;
								try {
									double value = deltaTime;
									os = new FileWriter(experimentDirectory + "/TM", true);
									//System.out.println(os);
									os.write("" + value + "\n");
									os.close();
								} catch (IOException ex) {
							...
							if (indicatorList_[j].equals("GD")) {
								FileWriter os = null;
								try {
									double value = indicators.getGD(resultFront);
									os = new FileWriter(experimentDirectory + "/GD", true);
									os.write("" + value + "\n");
									os.close();
								} catch (IOException ex) {
									Logger.getLogger(Experiment.class.getName()).log(Level.SEVERE, null, ex);
							...
							if (indicatorList_[j].equals("CMDeb")) {
								FileWriter os = null;
								try {
									double value = indicators.getConvergenceMeasure(resultFront);
									os = new FileWriter(experimentDirectory + "/CMDeb", true);
									os.write("" + value + "\n");
									os.close();
								} catch (IOException ex) {
									Logger.getLogger(Experiment.class.getName()).log(Level.SEVERE, null, ex);
							...
							if (indicatorList_[j].equals("RHV")) {
								
								if(hypervolume == null)
									throw new RuntimeException("Error in RHV");
								
								FileWriter os = null;
								try {
									double value = indicators.calculateHypervolumeRatio(hypervolume);
									os = new FileWriter(experimentDirectory + "/RHV", true);
									os.write("" + value + "\n");
									os.close();
								} catch (IOException ex) {
									Logger.getLogger(Experiment.class.getName()).log(Level.SEVERE, null, ex);
